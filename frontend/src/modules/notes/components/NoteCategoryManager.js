...
import { fetchNoteCategories, addNoteCategory, editNoteCategory, removeNoteCategory, updateNoteCategoryOrder } from '../../../store/categoriesSlice';
...
function SortableCategoryItem({ category, editingCategory, onEdit, onDelete }) {
  ...
  return (
    <div className="col-12 col-md-6" ref={setNodeRef} style={style}>
      <div
      ...
      >
        <div className="d-flex align-items-center flex-grow-1 overflow-hidden gap-2" style={{ minWidth: 0 }}>
          <div
            {...attributes}
            {...listeners}
            className="text-secondary opacity-50 cursor-grab hover-opacity-100 flex-shrink-0"
...
export default function NoteCategoryManager({ show, onClose, onCategoryUpdate }) {
  ...
  const loadCategories = useCallback(async (showSpinner = true) => {
    try {
      ...
      const data = await dispatch(fetchNoteCategories()).unwrap();
      setCategories(data || []);
    } catch {
  ...
  const handleSaveCategory = async (e) => {
    ...
    try {
      setIsSaving(true);
      if (editingCategory) {
        await dispatch(editNoteCategory({
          id: editingCategory,
          data: {
            name: trimmedName,
            color: selectedColor
        ...
        await dispatch(addNoteCategory({
          name: trimmedName,
          color: selectedColor
        })).unwrap();
  ...
  const confirmDelete = async () => {
    ...
    try {
      await dispatch(removeNoteCategory(categoryToDelete.id)).unwrap();
      toast.success('Category deleted');
      const updated = await dispatch(fetchNoteCategories()).unwrap();
  ...
  const handleDragEnd = async (event) => {
    ...
    if (active.id !== over.id) {
      ...
      try {
        ...
        await dispatch(updateNoteCategoryOrder(categoryOrders)).unwrap();
        if (onCategoryUpdate) onCategoryUpdate();
      } catch {
      ...
      <DeleteConfirmModal
        ...
        message={
          <>
            ...
            Notes using it will lose this label.<br />
